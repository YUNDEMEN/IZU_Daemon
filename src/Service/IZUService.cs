	...
	public class IZUService : IIZUService
	{
		...
		public IZUService(ILogger<IZUService> logger, IOptions<IZUConfig> cfg, IS7NetService s7netService)
		{
			...
			ServiceRuntime = new ServiceRuntime
			{
				Name = Config.Name,
				IP = Config.Server,
				RecoverySeconds = $"{Config.RecoverySeconds} s",
				RefreshInterval = $"{Config.RefreshMillionSeconds} ms"
			};
		...
		public async Task StartAsync()
		{
			...
			using (HttpClient httpClient = new HttpClient())
			{
				try
				{
					...
					httpClient.BaseAddress = new Uri(Config.OHTCServer);
					httpClient.DefaultRequestHeaders.Accept.Add(new System.Net.Http.Headers.MediaTypeWithQualityHeaderValue("application/json"));

					HttpResponseMessage response = await httpClient.GetAsync($"izu/exist?n={Config.Name}");
					...
					if (izu_id == 0)
					{
						response = await httpClient.PostAsync($"izu/add", JsonContent.Create(new { name = Config.Name, ip = Config.Server }));
						if (response.EnsureSuccessStatusCode().IsSuccessStatusCode)
						{
							string result = await response.Content.ReadAsStringAsync();
							var jsonResult = JsonObject.Parse(result);
